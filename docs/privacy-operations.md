...
1. Confirmar com a igreja que acompanha esse telefone e encaminha os pedidos. Sua indicação como contato não é, por si só, uma nomeação formal de encarregado. Verificar a obrigação de nomeação ou o enquadramento em eventual dispensa; cem usuários não comprovam dispensa.
...
1. Quem identificar a suspeita avisa o responsável. Ele aciona o administrador técnico e a representação da igreja. Registrar quando houve ciência, preservar evidências com acesso restrito e conter a falha — por exemplo, revogar sessões ou credenciais comprometidas. Não divulgar tokens, senhas ou cópias do banco.
